...
from Util.fix_point import Quantizer
...
# index - [0-15]
models = ["dists_mse_gan2", "mse400", "mse800", "mse1600", "mse3200", "mse6400", "mse12800", "mse25600",
...
def encode(im_dir, out_dir, model_dir, model_index, block_width, block_height):
    ...
    for img in img_block_list:
        ...
        for sample in samples:
            ...
            lowers_ = [list(map(ops.Decimal_cdf, sample_, mean_, scale_)) for mean_, scale_ in
                       zip(means_, scales_)]  # [[lower0], [lower1], [lower2]]
...
def decode(bin_dir, rec_dir, model_dir, block_width, block_height):
    ...
    for i in range(Block_Num_in_Height):
        for j in range(Block_Num_in_Width):
            ...
            with open(bin_dir + "main.bin", 'wb') as f:
                bits = file_object.read(FileSizeMain)
                f.write(bits)
            with open(bin_dir + "hyper.bin", 'wb') as f:
            ...
            cdf_h = cdf_h.astype(np.int) + sample.astype(np.int) - Min_V_HYPER
            T2 = time.time()
            AE.init_decoder(bin_dir + "hyper.bin", Min_V_HYPER, Max_V_HYPER)
            Recons = []
            ...
            if GPU:
                y_main_q = y_main_q.cuda()
            AE.init_decoder(bin_dir + "main.bin", Min_Main, Max_Main)
            ...
            for i in range(c_main):
                ...
                for j in range(int(block_H_PAD / 16)):
                    ...
                    x1 = F.conv3d(quantizer(y_main_q[:, :, i:i + 11, j:j + 11, :]), weight=quantizer(c_weight),
                                  bias=quantizer(c_bias))  # [1,24,1,1,1]
                    ...
                    lowers_ = [list(map(partial_cdf, mean_, scale_)) for mean_, scale_ in
                               zip(means_, scales_)]  # [N, len(Data), len(samples)]
                    ...
                    for k in range(int(block_W_PAD / 16)):
                        ...
                        if DEBUG:
                            ...
                            if r.all():
                                ...
                                print("FOUND NOT EQUAL in CDF !!! =====>", i, j, k, '\n', "decoded:", ecdf[index, int(
                                    ey_main_q[0, i, j, k] - Min_Main):int(
                                    ey_main_q[0, i, j, k] - Min_Main + 2)] - samples.astype(np.int)[
                                                                             int(ey_main_q[0, i, j, k] - Min_Main):int(
                                                                                 ey_main_q[
                                                                                     0, i, j, k] - Min_Main + 2)] + Min_Main,
                                      '\n', "origin :", cdf_m[k, int(ey_main_q[0, i, j, k] - Min_Main):int(
                                        ey_main_q[0, i, j, k] - Min_Main + 2)] - samples.astype(np.int)[int(
                                        ey_main_q[0, i, j, k] - Min_Main):int(
                                        ey_main_q[0, i, j, k] - Min_Main + 2)] + Min_Main)
                                print("=== Latent Features (E/D):", i, j, k, ey_main_q[0, i, j, k],
    ...
    os.system('rm ' + bin_dir + "hyper.bin")
    os.system('rm ' + bin_dir + "main.bin")
    img.save(rec_dir)
